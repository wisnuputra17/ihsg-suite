...
export function renderIep() {
  ...
  container.innerHTML = `
    <table class="data-table">
      ...
      <tbody>
        ${rows.map(r => `
          <tr class="${(r.surge || 0) >= 5 ? 'row-highlight' : ''}">
            <td class="sym-cell">${r.sym}</td>
            <td class="num-cell">${_fmtPrice(r.price)}</td>
            <td class="num-cell">${_fmtVol(r.vol)}</td>
            <td class="num-cell ${(r.surge || 0) >= 0 ? 'text-green' : 'text-red'}">
              ${r.surge !== null ? (r.surge >= 0 ? '+' : '') + r.surge.toFixed(2) + '%' : '–'}
            </td>
            <td class="num-cell">${r.ma10 ? _fmtPrice(r.ma10) : '–'}</td>
