...
const IEP_FETCH_DAYS  = 25      // fetch lebih banyak untuk buffer weekend + MA10 warmup
...
function _extractIEP(candles, dates) {
  ...
  // Build array IEP sesuai urutan dates
  const iepArr = dates
    .map(date => {
      const d = byDay[date]
      return d ? { date, price: d.price, vol: d.vol } : null
    })
    .filter(Boolean)

  if (!iepArr.length) return []

  // Hitung surge (% perubahan vs hari sebelumnya)
  for (let i = 0; i < iepArr.length; i++) {
    const prev = iepArr[i + 1]   // index lebih tinggi = hari lebih lama
    if (prev && prev.price > 0) {
      iepArr[i].surge = (iepArr[i].price - prev.price) / prev.price * 100
    } else {
      iepArr[i].surge = null
    }
  }

  // Hitung MA10 dari harga IEP
  const prices = iepArr.map(d => d.price)
  const ma10   = calcSMA(prices, 10)
  iepArr.forEach((d, i) => { d.ma10 = ma10[i] })

  return iepArr  // terbaru di index 0
}
