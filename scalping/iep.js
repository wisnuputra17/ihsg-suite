...
function _extractIEP(candles, dates) {
  ...
  // Hitung surge vs hari sebelumnya (seluruh data)
  for (let i = 0; i < allDays.length; i++) {
    const prev = allDays[i + 1]
    if (prev && prev.price > 0) {
      allDays[i].surge = (allDays[i].price - prev.price) / prev.price * 100
    } else {
      allDays[i].surge = null
    }
  }

  // Hitung MA10 dari seluruh data — warmup cukup
  const prices = allDays.map(d => d.price)
  const ma10   = calcSMA(prices, 10)
  allDays.forEach((d, i) => { d.ma10 = ma10[i] })

  // Ambil IEP_DAYS hari terbaru setelah MA10 terhitung
