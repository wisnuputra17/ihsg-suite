...
export async function fetchDaily(sym, from, to) {
  ...
  return raw.map(c => ({
    date:        c.date,                    // YYYY-MM-DD string — pakai ini sebagai time di chart
    unixdate:    toNum(c.unixdate),
    open:        toNum(c.open),
    high:        toNum(c.high),
    low:         toNum(c.low),
    close:       toNum(c.close),
    volume:      toNum(c.volume),
    foreignbuy:  toNum(c.foreignbuy),      // ⚠️ bukan foreign_buy (beda dari intraday)
    foreignsell: toNum(c.foreignsell)
  }))
}
...
export async function fetchIntraday(sym, fromTs, toTs, mult = 5) {
  ...
  return raw.map(c => {
    // Konversi datetime string ke unix timestamp WIB
    const parts = c.datetime.split(' ')
    const ts = Math.floor(new Date(parts[0] + 'T' + parts[1] + '+07:00').getTime() / 1000)
    return {
      datetime:     c.datetime,              // "YYYY-MM-DD HH:MM:SS"
      unix:         ts,
      open:         toNum(c.open),
      high:         toNum(c.high),
      low:          toNum(c.low),
      ...
      foreign_buy:  toNum(c.foreign_buy),    // ⚠️ beda dari daily (foreign_buy vs foreignbuy)
